...
1. 规则 / Skill / Harness 优先解析。
2. 本地 DuckDB 分析缓存优先查询。
3. 本地结构化酒店经营解释模板优先生成回答。
4. 大模型只在低置信、澄清、报告或显式开启增强时介入。
...
- `auto`：默认策略。规则解析高于阈值时跳过快模型；低置信或需要澄清时调用快模型。
- `always`：每次语义解析都调用快模型。
- `off`：完全不调用快模型，只用规则、Skill 和实体目录。

例如“看一下江门嘉华酒店3月的经营情况”会命中 `hotel_operation_overview`，并解析出 `202603`、酒店范围和预算对比口径，因此默认不再等待快模型。
...
- `off`：默认策略。普通问答使用本地结构化模板，响应最快。
- `auto`：报告、归因或显式深度解释场景调用深度模型。
